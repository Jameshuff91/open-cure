#!/usr/bin/env python3
"""
Evaluate TxGNN models using direct DistMult scoring.
Bypasses model.predict() by directly using embeddings and relation weights.

Usage (on Vast.ai GPU):
    python3 txgnn_eval_direct.py
"""
...
def get_embeddings_and_rel_weight(model):
    """
    Get node embeddings and relation weight for indication.

    Returns:
        disease_emb: (n_diseases, dim) array
        drug_emb: (n_drugs, dim) array
        rel_weight: (dim,) array for rev_indication (disease -> drug)
    """
    model.G = model.G.to(model.device)
    model.model.eval()

    with torch.no_grad():
        # Get embeddings from model forward pass
        h = model.model(model.G, model.G, return_h=True)

    disease_emb = h['disease'].cpu().numpy()
    drug_emb = h['drug'].cpu().numpy()

    # Get relation weight for rev_indication (disease -> drug direction)
    # The predictor has rel2idx and W
    rel2idx = model.model.pred.rel2idx
    W = model.model.pred.W

    # Find the rev_indication relation index
    # The key format is ('disease', 'rev_indication', 'drug')
    rev_ind_key = None
    for key in rel2idx.keys():
        if 'rev_indication' in str(key):
            rev_ind_key = key
            break

    if rev_ind_key is None:
        # Try indication (drug -> disease) and reverse our scoring
        for key in rel2idx.keys():
            if 'indication' in str(key) and 'rev' not in str(key):
                rev_ind_key = key
                print(f"Using indication relation (will need to reverse): {rev_ind_key}")
                break

    if rev_ind_key is None:
        raise ValueError(f"Could not find indication relation. Available: {list(rel2idx.keys())}")

    rel_idx = rel2idx[rev_ind_key]
    rel_weight = W[rel_idx].detach().cpu().numpy()

    print(f"Relation: {rev_ind_key}, idx: {rel_idx}")
    print(f"Disease embeddings: {disease_emb.shape}")
    print(f"Drug embeddings: {drug_emb.shape}")
    print(f"Relation weight: {rel_weight.shape}")

    return disease_emb, drug_emb, rel_weight


def evaluate_model(model, gt_data, disease_name_to_idx, drug_name_to_idx, all_drug_indices, model_name="Model"):
    """
    Evaluate model using direct DistMult scoring.
    """
    print(f"\n{'='*60}")
    print(f"Evaluating: {model_name}")
    print(f"{'='*60}")

    # Get embeddings and relation weight
    print("  Getting embeddings and relation weights...")
    disease_emb, drug_emb, rel_weight = get_embeddings_and_rel_weight(model)
    ...
    for disease_name, disease_data in gt_data.items():
        ...
        try:
            ...
            d_emb = disease_emb[disease_idx]  # (dim,)

            # Get all drug embeddings
            drug_embs = drug_emb[all_drug_indices]  # (n_drugs, dim)

            # DistMult scoring: score = sum(h_disease * r * h_drug)
            scores = np.sum(d_emb * rel_weight * drug_embs, axis=1)
