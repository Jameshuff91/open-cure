#!/usr/bin/env python3
"""h1275: 20-seed weight sweep on SUBSET_D_GLOBAL (supersedes 5-seed h1268).

h1269's 20-seed extension of h1264 locked SUBSET_D_GLOBAL (every disease
blended, w=0.5) as the canonical fusion recipe. h1268 preregistered a
fine-grained 5-seed weight sweep on that subset, but h1269 also showed that
5-seed paired-t over-estimates fusion effect sizes by 27-54% (FLIPPED
+0.00054 → +0.00025; GLOBAL +0.00469 → +0.00343). At 20 seeds the optimum
may differ from w=0.5.

This script runs a 5-point weight sweep on the locked GLOBAL subset using
the h1273 20-seed scaffold:

    blended(d) = w * z(n2v_score_d) + (1 - w) * z(concat_l2_score_d)
    w ∈ {0.3, 0.4, 0.5, 0.6, 0.7}

Two paired comparisons per weight:
  (a) vs concat_l2_raw anchor — does it still beat raw at p<0.05?
  (b) vs w=0.5 reference      — does any w ≠ 0.5 beat the current canonical?

Promotion rule (preregistered in research_roadmap.json h1275):
  if any w beats w=0.5 on Δ per-disease AUPRC at p<0.05 AND Δ > +0.0003,
  lock as new canonical weight.

Outputs:
    data/analysis/h1275_global_weight_sweep.{json,md}
"""
...
OUT_JSON = PROJECT_ROOT / "data/analysis/h1275_global_weight_sweep.json"
OUT_MD = PROJECT_ROOT / "data/analysis/h1275_global_weight_sweep.md"

ANCHOR = "concat_l2_raw"
WEIGHTS: Tuple[float, ...] = (0.3, 0.4, 0.5, 0.6, 0.7)
REFERENCE_W = 0.5
...
def main() -> None:
    ...
    print("=" * 72)
    print(f"h1275: 20-seed weight sweep on SUBSET_D_GLOBAL")
    print(f"  weights={WEIGHTS}  reference={ref_mode}  seeds={len(seeds)}")
    print("=" * 72)
    ...
    print("\n" + "=" * 72)
    print(f"PAIRED-T vs {ref_mode} (current canonical w={REFERENCE_W}, n={len(seeds)})")
    print("=" * 72)
    ...
    # Promotion gate
    print("\n" + "=" * 72)
    print(f"PROMOTION GATE: w ≠ 0.5 beats w=0.5 on Δ per-dis AUPRC at p<0.05 AND Δ>+0.0003")
    print("=" * 72)
    ...
    for w in WEIGHTS:
        ...
        passes = (pa["mean"] > 0.0003) and (pa["p"] < 0.05)
        promotion[m] = {
            "weight": w,
            "delta_per_dis_auprc": pa["mean"],
            "p": pa["p"],
            "passes_gate": bool(passes),
            "decision": f"PROMOTE w={w} over w={REFERENCE_W}" if passes else f"STAY with w={REFERENCE_W}",
        }
        print(f"  {m} (w={w}): Δ={pa['mean']:+.5f}  p={pa['p']:.4g}  →  {promotion[m]['decision']}")
    ...
    # Markdown
    md: List[str] = []
    md.append("# h1275 — 20-seed weight sweep on SUBSET_D_GLOBAL (soft-blend)\n\n")
    md.append(f"**Premise:** h1269's 20-seed extension locked `SUBSET_D_GLOBAL` with w={REFERENCE_W} ")
    md.append("as the canonical fusion recipe. h1268 preregistered a weight sweep at 5 seeds; ")
    md.append("h1269 showed 5-seed paired-t over-estimates effect sizes by 27-54%. This script runs ")
    md.append(f"the 20-seed sweep on the locked subset with w ∈ {list(WEIGHTS)}.\n\n")
    ...
    md.append(f"\n## Paired-t vs `{ref_mode}` (current canonical w={REFERENCE_W}, n={len(seeds)})\n\n")
    md.append("| Mode (w) | ΔR@30 (p) | Δper-dis-AUPRC (p) | Δper-dis-AUROC (p) |\n")
    md.append("|---|---|---|---|\n")
    for w in WEIGHTS:
        if w == REFERENCE_W:
    ...
    md.append(f"\n## Promotion gate (beat w={REFERENCE_W} on Δ per-dis AUPRC at p<0.05, Δ>+0.0003)\n\n")
    md.append("| Mode (w) | Δ vs w=0.5 | p | Decision |\n|---|---|---|---|\n")
    for m, info in promotion.items():
        md.append(f"| `{m}` (w={info['weight']}) | {info['delta_per_dis_auprc']:+.5f} | {info['p']:.3g} | **{info['decision']}** |\n")
