...
**H961 MESH DISEASE-NAME ALIASING (VALIDATED):** 668 disease_names entries missing from mesh_mappings characterized by alias-generator (`scripts/h961_alias_generator.py`) + reverse-index backfill. Principled transformations (US/UK spelling, hyphen-insertion for compound prefixes, possessive restoration, plural stripping, trailing-abbrev drop) resolve only **53/668 (7.9%) at 83% precision** vs disease_names — far short of the 90% target. Rule breakdown: trailing_abbrev 10/10 (100%), plural 18/21 (86%), possessive 6/8 (75%), disease_suffix 4/6 (67%), british_spelling 3/5 (60%). **Less-specific-prefix fallback REJECTED** as a dangerous coarsening: 43% of its 207 would-be hits pointed to a DIFFERENT id than disease_names (e.g. `allergic dermatitis` → `dermatitis` D003872 instead of the correct atopic-dermatitis D017449). Root cause of the miss: 408/668 are name-granularity mismatches where mm has a shorter canonical form (mm `acne` D000152 vs dn `acne vulgaris`), 260/668 have ids absent from mm entirely (C-numbered supplementary records or h901-descended entries). **Combined fix shipped:** disease_names reverse-index backfill (668/668 = 100% safe, authoritative) + 114 reverse British-spelling variants for existing mm keys (sleep apnoea, oedema, coeliac disease, hypercholesterolaemia, …). mesh_mappings grew **1489 → 2265** at load time. Integration: `data/reference/h961_disease_name_aliases.json` loaded in `src/production_predictor.py:2221-2244`; h952 runtime fallback remains as a safety net for out-of-band names. Smoke test passes on 7/7 previously-failing holdout queries including British spellings (autoimmune haemolytic anaemia, obstructive sleep apnoea). Lesson: validate every alias-generator hit against an authoritative id source; a wrong-id alias is worse than no alias.
...
**H979 CANCER_SAME_TYPE_MECH_RANK10 HIGH→MEDIUM (VALIDATED):** Rule holdout 72.5% post-h952-fix (h975), 2.2σ below HIGH mean 79.9%. Demoted to MEDIUM. **Isolated effect** (measured by reverting and re-running h393 on top of h986 baseline): GOLDEN 80.8→82.6% (**+1.8pp** — STRONG-lit cancer_same_type_mech_rank10 preds cascade MEDIUM→GOLDEN via literature_strong_promotion at line 4959), HIGH 78.8→80.4% (**+1.6pp** — meets the +1pp h979 criterion cleanly), MEDIUM 39.5→39.9% (+0.4pp), LOW/FILTER unchanged. Combined with h986 vs pre-h986 baseline: GOLDEN 78.6→82.6% (+4.0pp), HIGH 79.9→80.4% (+0.5pp). **Methodology lesson (h987 filed):** the h979 simulation (post-hoc tier-remap of cached counters) predicted +1.44pp HIGH / 0pp GOLDEN — the cascade was invisible to post-hoc remapping. Actual isolated was +1.6pp HIGH / +1.8pp GOLDEN. Future demotion audits must either run the real predictor or replay the tier post-processing pipeline (lines 4943–5022). Raw: `data/analysis/h979_isolated_revert.txt`, `data/analysis/h979_final_restored.txt`.
**H953 BIOLOGIC PRECISION PIVOT (INVALIDATED):** Per-category biologic prior (= train-only fraction of diseases in category with ≥1 biologic in expanded GT) cannot drive a precision-aware demotion. 5-seed h393 holdout, top_n=200, thresholds {0.05, 0.10, 0.15, 0.20, 0.30}: baseline p30=14.32%±0.94%, r30=19.79%±1.59%, bio_r30=30.31%±3.57% (avg 3.4 biologics in top-30). Best p30 lift = **+0.05pp** at thr=0.30 — far below the +2pp ship gate. At thr=0.10 only `infectious` (prior=0.088) qualifies; demotion is precision-NEUTRAL (per-category p30 Δ=0.00pp; bio_r30 -0.18pp net). At thr≥0.15 demotion expands to `other`/`psychiatric`/`dermatological` and costs bio_r30 -5.6 to -8.2pp without moving p30. **Three-part mechanism:** (1) baseline kNN already concentrates biologics in immunology/cancer/endocrine — few are misplaced; (2) the rank-31+ non-biologic that fills the slot hits at the same background rate as the demoted biologic, so the swap is precision-neutral; (3) category-prior is too coarse — `dermatological` prior=0.183 but bio_r30=72.2%, so any demotion there destroys real biologic hits. **Side finding (h991-worthy):** baseline bio_r30=30.31% > overall_r30=19.79% (+10.5pp) — biologics OVERPERFORM, so the precision pivot must operate on small-molecule slots, not biologic slots. Raw: `data/analysis/h953_biologic_precision_pivot.json`, `data/analysis/h953_run.txt`. Follow-ups filed: h990 (per-disease kNN-neighbor prior, finer-grained), h991 (small-molecule precision gap, inverse pivot), h992 (p30 ceiling = min(|GT|,30)/30 — metric-hygiene reframe).
